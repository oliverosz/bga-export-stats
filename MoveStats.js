javascript:{
    ...
    function exportStats() {
        ...
        for (var i = 0; i < logs.length; i++) {
            ...
            else if (player == "" && logs[i].className.includes("gamelogreview")) {
                var words = logs[i].innerText.split(" ");
                player = words.find(word => players.includes(word));
                output = output + [tableID, gameName, moveNo, JSDateToExcelDate(new Date(date + time)), player].join(";") + "\n";
            }
        }
        ...
        if (div == null) {
            ...
            header.innerText = "Table ID;Game Name;Move No.;Date Time;Player Name"; /* column headers in the box title */
